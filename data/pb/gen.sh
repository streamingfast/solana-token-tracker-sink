...
PROTO_HIVEMAPPER=${2:-"$ROOT/../solana-token-tracker/proto"}
...
function main() {
  ...
  generate "solana-token-tracker/v1/solana-token-tracker.proto"
...
function generate() {
    ...
    for file in "$@"; do
      protoc -I$PROTO_HIVEMAPPER \
        --go_opt=Msolana-token-tracker/v1/solana-token-tracker.proto=github.com/streamingfast/solana-token-tracker/data/pb \
